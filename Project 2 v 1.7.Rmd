...
```
\
Left out the following variables: phy_id, age, weight, bonetreat, bonemed, smoke, premeno. Although, through EDA age seems to be an important variable so will keep this in, in future models.  
\
...
# plot ROC
plot(complex1.roc,print.thres="best",col="red", main = "Best threshold for training data set for complex model")
...
# plot ROC
plot(complex1.roc.Valid,print.thres="best",col="red", main = "Best threshold for validation data set Complex Model")

# Best threshold for validation is 0.219
auc(complex1.roc.Valid) # 0.7312
```
...
plot_model(complex1,type="pred",terms=c("fracscore","age[55,65,75,90]", "fracture")) # shows predictive probability of fracturing based on bonetreat and age, surprisingly lower ages are more likely to break a bone as fracscore increases, maybe due to mobility

plot_model(complex1,type="pred",terms=c("fracture","age[55,65,75,90]"))
...
# create site_id as factor
trainingDataframe$site_id_factor <- factor(trainingDataframe$site_id)
testingDataframe$site_id_factor <- factor(testingDataframe$site_id)

#Version 1
lda.fit<-train(fracture ~ . - sub_id - phy_id - site_id + priorfrac:fracscore + age:fracscore + fracscore:bonetreat,
               data=trainingDataframe,
               method="lda",
               trControl=fitControl,
...
# check confusion matrix positive class is yes fracture
confusionMatrix(data = validatePredictions, reference = testingDataframe$fracture, positive="Yes")


# plot ROC for LDA validation set
plot(ldafit.roc.valid,print.thres="best",col="lightblue", main = "Best threshold for LDA validation data set")

# Best threshold for validation is 0.219
auc(ldafit.roc.valid) # 

# Plot both complex and lda models
plot(complex1.roc.Valid,print.thres="best",col="red")
plot(ldafit.roc.valid, col="lightblue", add = T, legend = T, print.thres="best")
 legend("bottomright",
...
randomForestModel<-train(fracture ~ . - sub_id - phy_id,
                    data=trainingDataframe,
                    method="ranger",
                    trControl=fitControl,
                    preProc = c("center", "scale"))
...
# get AUC-ROC for RF
auc(rf.result.roc) # AUC = 0.7706

```

```{r}
# compare all object 2 models with AUC-ROC metrics (higher is better), so in this case the RF model was best
auc(complex1.roc.Valid) # AUC 0.7312
auc(ldafit.roc.valid) # AUC = 0.7414
auc(rf.result.roc) # AUC = 0.7706
```

```{r}
# Single versions of ROC plots for powerpoint
plot(complex1.roc.Valid, print.thres="best", main = "Complex model ROC")
plot(ldafit.roc.valid, print.thres="best", main = "LDA model ROC")
plot(randomForestModel, print.thres="best", main = "Random Forest model ROC")
```


```{r}
 ######## Another Way to do LDA - worked then stopped working..
# library(ROCR)
# library(MASS)
# lda2 <- lda(fracture ~ . -sub_id - phy_id + priorfrac:fracscore + age:fracscore + fracscore:bonetreat, data=trainingDataframe)
